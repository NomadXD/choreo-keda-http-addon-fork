...
func NewUpstream(roundTripper http.RoundTripper, retryCount int, logger logr.Logger) *Upstream {
	return &Upstream{
...
func (uh *Upstream) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if i := strings.Index(r.Host, ":"); i != -1 {
		...
		if stream, err = url.Parse(targetHost); err != nil {
			uh.logger.Error(err, "forwarding failed")
			w.WriteHeader(500)
			if _, err := w.Write([]byte(fmt.Sprintf("error parsing host:port: %s to URL", targetHost))); err != nil {
				uh.logger.Error(err, "could not write error response to client")
			}
			return
		}
	}
	...
	proxy.ModifyResponse = func(resp *http.Response) error {
		if resp.StatusCode == http.StatusServiceUnavailable {
			...
			if bytes.HasPrefix(buf.Bytes(), []byte("upstream connect error or disconnect/reset before headers")) {
				updatedResp, err := retryRequest(uh.logger, resp, uh.retryCount, 0)
				if err != nil {
					return err
				}
				*resp = *updatedResp
